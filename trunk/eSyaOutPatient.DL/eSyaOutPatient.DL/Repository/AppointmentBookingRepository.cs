            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            var p_exists = db.GtEfoppr.Where(w => w.MobileNumber == obj.PatientMobileNumber && w.Isdcode == obj.Isdcode).Count();
                            if (p_exists > 0)
                            {
                        ...
                        var dc_ap = await db.GtEcdcbn
                                        .Where(w => w.BusinessKey == obj.BusinessKey
                                            && w.FinancialYear == financialYear
                                            && w.DocumentId == DocumentIdValues.opc_AppointmentId).FirstOrDefaultAsync();
                        ...
                        return new DO_ReturnParameter { Status = true, Key = appointmentKey.ToString() };
                    }
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        // Check if mobile is used by another patient
                        if (app_dt.Isdcode.ToString() + app_dt.MobileNumber.ToString() != obj.Isdcode.ToString() + obj.PatientMobileNumber.ToString())
                        {
                            var p_exists = db.GtEfoppr.Where(w => w.MobileNumber == obj.PatientMobileNumber && w.Isdcode == obj.Isdcode).Count();
                            if (p_exists > 0)
                            {
