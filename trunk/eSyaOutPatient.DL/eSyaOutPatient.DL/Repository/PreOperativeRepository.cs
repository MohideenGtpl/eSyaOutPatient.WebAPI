...
namespace eSyaOutPatient.DL.Repository
{
    public class PreOperativeRepository : IPreOperativeRepository
    {
                            ...
                            else
                            {
                                if (!string.IsNullOrEmpty(c.Value))
                                {
                                    ...
                                    if (c.CLControlID == "FS1-1" || c.CLControlID == "FS3-1" || c.CLControlID == "FS4-1")
                                    {
                                        ...
                                        //Allow App Functions
                                        var pt = db.GtEfoppr.Where(w => w.RUhid == obj.UHID).FirstOrDefault();
                                        if(pt !=null)
                                        {
                                            pt.IsChatApplicable = true;
                                            pt.IsGuideApplicable = true;
                                        }
                ...
                if (fType == "All")
                {
                    var r = await db.GtOpclin
                        ...
                        .Select(x => new DO_PreOperativeInformation
                        {
                            ...
                            VisitNumber = Convert.ToInt32(x.Vnumber),
                            StrCreatedBy= db.GtEuusms.Where(w => w.UserId == x.CreatedBy).FirstOrDefault().LoginId,
        ...
        public async Task<List<DO_ControlValue>> GetSurgeries(int businessKey, int uhid)
        {
            using (var db = new eSyaEnterprise())
            {
                var sur_list = new List<DO_ControlValue>();
                var exist = await db.GtOpclin
                        ...
                        .ToListAsync();
                if(exist != null)
                {foreach (var sur in exist)
                    {
                        var sur_name =  db.GtOpclin
                        .Where(w => w.BusinessKey == businessKey && w.Uhid == uhid && w.TransactionDate==sur.TransactionDate && (w.ClcontrolId == "FS4-5-18" || w.ClcontrolId == "FS3-5"))
                        .Select(s => new DO_ControlValue
                        {
                            Value = s.Value,
                        }
                        ...
                        if(sur_name != null)
                        {
                            sur_list.Add(new DO_ControlValue
                            {
                                TransactionDate = Convert.ToDateTime(sur.Value),
                                Value = sur_name.Value
