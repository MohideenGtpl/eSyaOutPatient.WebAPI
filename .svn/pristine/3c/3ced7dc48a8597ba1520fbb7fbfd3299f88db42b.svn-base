...
namespace eSyaOutPatient.DL.Repository
{
    public class OPRegistrationRepository : IOPRegistrationRepository
    {
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if(app_sr == null)
                        {
                            var sr = db.GtEsclst.Where(w => w.BusinessKey == obj.BusinessKey && w.ClinicId == app_h.ClinicId && w.ConsultationId == app_h.ConsultationId
        ...
        public async Task<DO_ReturnParameter> InsertInToPatientMaster(DO_PatientData obj)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var exists = db.GtEfoppr.Where(w => w.MobileNumber == obj.MobileNumber && w.Isdcode==obj.Isdcode).Count();
                        if (exists > 0)
                        {
